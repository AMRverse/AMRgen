...
cooccuring_ppv_analysis <- function(geno_table, pheno_table, antibiotic, drug_class_list,
  ...
  # --- 2. Calculate Stats ---
  stats_R <- analysis_data %>%
    group_by(marker) %>%
    summarise(
      x = sum(R, na.rm = T), n = n(), p = x / n,
      se = sqrt(p * (1 - p) / n),
  ...
  g_matrix <- ggplot(matrix_data, aes(x = gene, y = marker)) +
    ...
    labs(x = "", y = "") +
    theme(
      axis.text.y = element_blank(), # Hide marker text (the dots ARE the text now)
  ...
  g_pheno <- ggplot(analysis_data, aes(y = marker, fill = pheno)) +
    ...
    theme(
      axis.text.y = element_blank(), # Hide Y labels (Matrix handles this)
      axis.ticks.y = element_blank(),
