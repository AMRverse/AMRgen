...
#' @param geno_drug (Optional) String (or vector of strings) specifying the drug name/s to filter on (default `NULL`). Filter is based on valid CARD terms in the EBI `antibiotic_name` field and is only used if `data` = `"genotype"` and neither `geno_subclass` nor `geno_class` is specified.
