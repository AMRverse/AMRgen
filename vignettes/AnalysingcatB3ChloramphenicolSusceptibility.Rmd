...
DASSIM_pheno <- btESBL_AST %>% 
  pivot_longer(names_to = "drug_agent", 
               values_to = "pheno", 
...
AST_pheno <- NCBI_Ecoli_AST_chl %>% filter(id %in% MICROBIGGE_Ecoli_CHLR$id) %>% interpret_ast(interpret_clsi=TRUE)
...
AST_CATB3_pheno <- AST_pheno %>% filter(id %in% MB_CATB3_geno$id) %>%
...
assay_by_var(pheno_table = AST_CATB3_pheno, 
             antibiotic = "CHL", 
             measure = "mic", 
             colour_by = "pheno_clsi",
             species = "Escherichia coli") + 
...
AST_CATB3_pheno_2 = MB_CATB3_geno %>% select(id, gene, `% Coverage of reference`) %>% 
  distinct(id, .keep_all = TRUE) %>% right_join(AST_CATB3_pheno, by = "id")
...
AST_CATB3_pheno_3 %>% group_by(truncation) %>% summarise(median=median(mic, na.rm=T), n=n(), R=sum(pheno_clsi=="S"))
```
