...
- species:
  ...
  default, the field `organism` in the input file will be assumed to
...
- ab:
  ...
  By default, the field `antibiotic` in the input file will be assumed
  to specify the antibiotic for each sample, but if this is missing or
  you want to override it in the interpretation step, you may provide a
  single antibiotic name via this parameter.

- source:

  (optional) A string to record as the source of these data points, e.g.
  `"EBI_browser"`. By default, the publications field (for EBI data) or
  BioProject field (for NCBI data) will be used to indicate the source
  for each row in the input file. For other input files, or to override
  this with a single value for all samples, you may provide a source
  name via this parameter.
...
- `method`: The AST method (e.g., `"broth dilution"`,
  ...
  based on the NCBI/EBI antibiogram specification.
