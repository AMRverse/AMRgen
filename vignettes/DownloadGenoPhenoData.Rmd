...
This function is fast but requires authentication via a [Google Cloud account](https://docs.cloud.google.com/docs/get-started) and may require payment. Free trial accounts can be set up, but require credit card authorization. Google currently provides enough free tier usage for >150 different queries for genotype data per month. To use this you will also need to install the `bigrquery` package and authorize it to use your Google cloud account.
...
# NOTE: you may need to add 'project_id="xxx"' to the command if you have not set up application default credentials
staph_ast_ncbi_cloud <- query_ncbi_bq_ast(staph_ast_ncbi_cloud_raw, interpret_clsi = TRUE)
```
