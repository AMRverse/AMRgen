---
title: "Downloading geno-pheno data from NCBI and EBI"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Downloading geno-pheno data from NCBI and EBI}
...
The `download_ncbi_pheno()` function lets you download antibiogram data from NCBI via their 'EUtils' API using the rentrez R pacakge. You must specify a species, and can optionally limit the download to one or more specific drugs. The function can also re-format the data into an AMRgen phenotype table, and re-interpret phenotypes against clinical breakpoints from EUCAST or CLSI. 
...
The `query_ncbi_bq_pheno()` function lets you download antibiogram data from NCBI via Google Cloud BigQuery using the bigrquery R pacakge. You must specify a species, and can optionally limit the download to one or more specific drugs. The function can also re-format the data into an AMRgen phenotype table, and re-interpret phenotypes against clinical breakpoints from EUCAST or CLSI. 

This function is fast but requires authentication via a [Google Cloud account](https://docs.cloud.google.com/docs/get-started) and may require payment. Free trial accounts can be set up, but require credit card authorisation. Google currently provides enough free tier usage for >150 different queries for genotype data per month. To use this you will also need to install the `bigrquery` package and authorize it to use your Google cloud account.
...
staph_ami_mic_plot
```
For more guidance on how to visulaise phenotypic data and combine it with genotypic data, have a look at the other `AMRgen` [vignettes](https://amrgen.org/articles/AnalysingGenoPhenoData.html).
...
For more examples on how to do join geno-pheno analyses and visualisations, so the other AMRgen [Vignettes](https://amrgen.org/articles/AnalysingGenoPhenoData.html).
