...
#' @param ecoff_col A character string specifying the column name in `pheno_table` that contains resistance interpretations made against the ECOFF rather than a clinical breakpoint. The values should be `"NWT"`, `"WT"` or otherwise interpretable by [AMR::as.sir()]. Default `ecoff`. Set to `NULL` if not available.  Only used if `binary_matrix` not provided.
