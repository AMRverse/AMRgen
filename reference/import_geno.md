...
common bioinformatics tools (AMRFinderPlus, ABRicate, Kleborate) as well
as processed AMRFinderPlus downloadable from EBI. Drug/class annotations
given for each genotype marker in the input file are parsed to standard
antibiotic names and/or antibiotic groups recognised by the AMR pkg, to
facilitate extracting relevant genotype markers for comparison to
phenotype data for a specific antibiotic (e.g. using `AMRgen` functions
...
- input:
  ...
  or the files may be generated using common bioinformatics software for
  AMR genotyping.

- format:

  A string indicating the format of the data: `"amrfp"` (default),
  `"ebi_web"`, `"ebi_ftp"`, `"kleborate"`, `"abricate"`. This determines
  which importer function the data is passed on to for processing (see
  below).
