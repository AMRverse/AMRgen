...
#' * `metrics`: A [tibble][tibble::tibble] listing concordance metrics, comparing observed vs predicted R and/or NWT calls, for all drugs with matched samples in both input tables.
#' * `drugs_R`: A long-form [tibble][tibble::tibble] listing the number of samples with each combination of observed and predicted binary calls for R, for all drugs with matched samples in both input tables.
#' * `drugs_NWT`: A long-form [tibble][tibble::tibble] listing the number of samples with each combination of observed and predicted binary calls for NWT, for all drugs with matched samples in both input tables.
...
concordance_from_tables <- function(pheno_table,
  ...
  if (!(sample_col %in% colnames(pheno_table) & sample_col %in% colnames(pheno_pred_table))) {
    stop(paste(sample_col, "must be present in both input tables"))
  }
  if (!(drug_col %in% colnames(pheno_table) & drug_col %in% colnames(pheno_pred_table))) {
    stop(paste(drug_col, "must be present in both input tables"))
  }
  ...
  if (!is.null(pred_ecoff) && pred_ecoff %in% colnames(obs_pred)) {
    if (!is.null(true_ecoff_col) && true_ecoff_col %in% colnames(obs_pred)) {
      # we have observed and pred for NWT, proceed
      obs_pred <- obs_pred %>%
      ...
      drug_list <- unique(drugs_NWT[[drug_col]])
      for (this_drug in as.character(drug_list)) {
        drug_data <- obs_pred %>%
          filter(!!sym(drug_col) == this_drug)
        # plot distribution of observed WT/NWT values for genomes called as WT (incl NA) or NWT
        bar_plot_NWT <- NULL
